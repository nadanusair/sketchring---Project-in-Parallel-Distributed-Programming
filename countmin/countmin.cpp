...
void CM_Print(CM_type * cm)
{
  ...
  for (int j=0;j<cm->depth;j++) {
      for (int i=0;i<cm->width;i++) {
        //outfile << cm->counts[j][i] << ",";
          std::cout << cm->counts[j][i] << ",";
      }
  }

  //outfile << std::endl; //comment-out if we want the sketch in one line in the csv file4
  std::cout << std::endl; //comment-out if we want the sketch in one line in the csv file4
}
